...
void Init_Digital_Inputs(void){
    ...
    pinMode(LIMIT6, INPUT_PULLDOWN);

    pinMode(SUPPLY_BUTTON_STATE, INPUT_PULLUP);
...
void Init_False_Trigger_ISR(){
  attachInterrupt(LIMIT1, False_Swtich_Trigger_1, RISING);
  ...
  attachInterrupt(LIMIT6, False_Swtich_Trigger_6, RISING);
  attachInterrupt(INPUT2, False_Swtich_Trigger_6, RISING);
}
