...
const int32_t J6_TOF_SPEED = 2000;
...
/// @brief Spin Joint 6 while INPUT1 is signaling "object detected" (driven by an external
/// Teensy running its own VL53L1X ToF sensor). Only acts when the robot is enabled and not
/// mid-homing, so it can't fight those routines.
void Handle_ToF_J6()
{
  if (PAROL6.disabled == 1) return;
  if (home_command == 1) return;

  int tof_triggered = digitalRead(INPUT1);

  static int last_state = -1;
  if (tof_triggered != last_state)
  {
    Serial.print("INPUT1 changed to: ");
    Serial.println(tof_triggered);
    last_state = tof_triggered;
  }

  if (tof_triggered == LOW)   // <-- changed from HIGH to LOW
  {
    stepper[5].setSpeed(0);
    stepper[5].runSpeed();
  }
